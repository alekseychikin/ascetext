class Autocomplete {
	constructor(plugins, selection, builder, editing) {
		...
		this.patterns = Object.keys(plugins).map((plugin) => {
			return plugins[plugin].autocompleteRule ? { plugin, rule: plugins[plugin].autocompleteRule } : null
		}).filter(Boolean)
		this.lastAnchorContainer
	...
	onSelectionChange(selection) {
		...
		let content = selection.anchorContainer.element.outerText
		let itemContent
		let start
		let index
		let match
