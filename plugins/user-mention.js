...
const SuggestBar = require('../utils/suggest-bar')
...
class UserMentionPlugin extends PluginPlugin {
	constructor(params = {}) {
		super()
	...
	autocomplete(match, selection, boundings) {
		...
		const items = suggests.filter((suggest) => {
			return suggest.label.indexOf(username) === 0
		})
...
module.exports.UserMentionPlugin = UserMentionPlugin
